...
import com.sushil.repositories.RefreshTokenRepository;
...
public class RefreshTokenService {
    ...
    public RefreshToken verifyExpiration(RefreshToken token){
        if (token.getExpiryDate().compareTo(Instant.now())<0){
            ...
            throw new TokenRefreshException(token.getToken(),"RefreshToken was expired. Please make a new signin request");
        }
        return token;
    }
    @Transactional
